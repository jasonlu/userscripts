...
(function () {
  ...
  let fc2DataSource: FC2PPVDataSource;
  ...
  document.addEventListener("mouseover", async function (e: MouseEvent) {
    ...
    if (target && target.tagName === "A") {
      ...
      const megnetLink = [
        ...(aElements as unknown as HTMLAnchorElement[]),
      ].find((a) => a.href.startsWith("magnet:"));
      currentMegnetLink = megnetLink?.href ?? "";
      if (match) {
        const videoId = match[1];
        if (fc2DataSource === null) {
          fc2DataSource = new FC2PPVDataSource();
        }
